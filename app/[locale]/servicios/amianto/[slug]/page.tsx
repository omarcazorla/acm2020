...
export function generateStaticParams() {
  return amiantoServices.map((s) => ({ slug: s.slug }))
}

export async function generateMetadata({ params }: Props) {
  const { locale, slug } = await params
  const service = amiantoServices.find((s) => s.slug === slug)
  if (!service) return {}
  const t = await getTranslations({ locale, namespace: 'services' })
  const key = `asbestos.${service.translationKey}`
  const metaDesc = t.has(`${key}.metaDescription`) ? t(`${key}.metaDescription`) : t(`${key}.description`)
  return {
    title: `${t(`${key}.title`)} | ACM-2020`,
    description: metaDesc,
    alternates: getAlternates(locale as Locale, '/servicios/amianto/[slug]', { slug }),
  }
}

export default async function AmiantoServicePage({ params }: Props) {
  const { locale, slug } = await params
  ...
  const tPages = await getTranslations({ locale, namespace: 'pages.asbestosIndex' })
    ...
    <>
      ...
      <Breadcrumbs
        items={[
          ...
          { label: tPages('title'), href: '/servicios/amianto' },
          { label: t(`${key}.title`) },
        ]}
      />
